...
SVM_FINAL_PROCESSED_DIR = PROJECT_ROOT / "data" / "svm_final_processed"
...
def add_svm_owner_metrics(report: dict) -> dict:
    tp = report["TP"]
    tn = report["TN"]
    fp = report["FP"]
    fn = report["FN"]
    tnr = tn / (tn + fp) if (tn + fp) else 0.0
    balanced_accuracy = (report["recall"] + tnr) / 2
    return {
        **report,
...
def run_experiment(name, X_train, y_train, X_test, y_test, learning_rate, lambda_param, n_epochs, batch_size):
    ...
    report = add_svm_owner_metrics(classification_report_binary(y_test, y_pred))
    row = {
...
def main():
    ...
    train_path = SVM_FINAL_PROCESSED_DIR / "train_processed.csv"
    test_path = SVM_FINAL_PROCESSED_DIR / "test_processed.csv"
    oversampled_path = SVM_FINAL_PROCESSED_DIR / "train_processed_oversampled.csv"
    for path in (train_path, test_path, oversampled_path):
        if not path.exists():
            raise FileNotFoundError(f"Missing restored SVM processed data file: {path}")

    # SVM intentionally uses the restored teammate-confirmed split instead of
    # regenerating preprocessing here. This preserves the SVM owner's final
    # experiment and prevents report/PPT/code-demo metric mismatches.
    ...
    for e in experiments:
        ...
        rows.append(row)
        if best is None or (row["f1"], row["balanced_accuracy"]) > (best["f1"], best["balanced_accuracy"]):
    ...
    print(f"SVM processed split: {SVM_FINAL_PROCESSED_DIR}")
